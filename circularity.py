            ...
            if (pixel_array[currentX + 1][currentY + dirY] == 255):
                ...
                if (pixel_array[currentX - 1][currentY + dirY] == 255):
                    ...
                    if (pixel_array[currentX - 1][currentY] == 255):
                        print("24")
                        # one pixel to left == white pixel, must be the next direction
                        loopAroundCircle(pixel_array, steps+1, currentX - 1, currentY, -1, 0, startX, startY)
                    elif (pixel_array[currentX + 1][currentY] == 255):
                        print("25")
                        # one pixel to right == white pixel, must be the next direction
                        loopAroundCircle(pixel_array, steps+1, currentX + 1, currentY, 1, 0, startX, startY)
                else:
                    print("26")
                    # same y direction, one to left = black pixel, must be same y direction, one to the right
                    loopAroundCircle(pixel_array, steps+1, currentX - 1, currentY + dirY, -1, dirY, startX, startY)
            else:
                print("27")
                # same y direction, one to right = black pixel, must be same y direction, one to the left
                loopAroundCircle(pixel_array, steps+1, currentX - 1, currentY + dirY, -1, dirY, startX, startY)
            ...
            if (pixel_array[currentX + dirX][currentY + 1] == 255):
                ...
                if (pixel_array[currentX + dirX][currentY - 1] == 255):
                    ...
                    if (pixel_array[currentX][currentY + 1] == 255):
                        print("4")
                        # one pixel to down == white pixel, must be the next direction
                        loopAroundCircle(pixel_array, steps+1, currentX, currentY + 1, 0, 1, startX, startY)
                    elif (pixel_array[currentX][currentY] == 255):
                        print("5")
                        # one pixel to up == white pixel, must be the next direction
                        loopAroundCircle(pixel_array, steps+1, currentX, currentY - 1, 0, -1, startX, startY)
                else:
                    print("6")
                    # same x direction, one up == black pixel, must be same y direction, one down
                    loopAroundCircle(pixel_array, steps+1, currentX + dirX, currentY + 1, dirX, 1, startX, startY)
            else:
                print("7")
                # same x direction, one down == black pixel, must be same x direction, one up
                loopAroundCircle(pixel_array, steps+1, currentX + dirX, currentY - 1, dirX, -1, startX, startY)
        # same direction = black pixel
            ...
            if (pixel_array[currentX][currentY + dirY] == 255):
                ...
                if (pixel_array[currentX + dirX][currentY] == 255):
                    ...
                    if (pixel_array[currentX - dirX][currentY] == 255):
                        ...
                        loopAroundCircle(pixel_array, steps+1, currentX + dirX, currentY, dirX, 0, startX, startY)
                    else:
                        print("16")
                        # opposite x direction == black pixel, must be y direction
                        loopAroundCircle(pixel_array, steps+1, currentX, currentY + dirY, 0, dirY, startX, startY)
                else:
                    print("17")
                    # x direction == black pixel, must be exact same direction
                    loopAroundCircle(pixel_array, steps+1, currentX + dirX, currentY + dirY, dirX, dirY, startX, startY)
            else:
                print("18")
...
def main():
    ...
    image = Image.open(os.path.join(script_dir, 'smallest.png'))
